...
A [CSV file](https://minus34.com/opendata/geoscape-202608/boundary_concordance.csv) and supporting scripts for converting data between Australian boundaries.
...
Download & import the ~50Mb [concordance file](https://minus34.com/opendata/geoscape-202608/boundary_concordance.csv) into your database or reporting tool to [get started](#get-started). A [script](/postgres-scripts/00_import_concordance_file.sql) for importing into Postgres is also provided.

### Example Use Cases

- You have sales data by ABS Census 2026 SA2 boundaries & competitor data by local government area (LGA) and need to determine market share. Using the boundary concordance file, you convert the SA2 data to LGA and merge both datasets by LGA ID.
...
- The code has been tested against the official ABS Census 2026 to 2021 boundary correspondences and the difference is: ~1% of addresses are in another boundary
    - **Note**: ABS Census 2026 to 2021 boundary conversions aren't in the concordance file yet.
...
![pc_vs_lga.png](pc_vs_lga.png "ABS 2026 Postcodes (red) vs LGAs (blue)")
...
In the [score](/data/boundary_concordance_score.csv) file, the **_error_** measures what proportion of data is placed in the wrong boundary when converting a statistic common to both boundaries, like population, from postcode to LGA (for example). _Note: error rates are only available for ABS 2026 from/to boundaries as the error check requires census data._

## Methodology

The concordance file is generated by the following process:

1. Tag all GNAF addresses with 2026 & 2021 ABS Census boundaries and geoscape 202608 Administrative boundaries
...
1. Download the [concordance file](https://minus34.com/opendata/geoscape-202608/boundary_concordance.csv)
...
1. The boundary from/to combination you need isn't in the standard [concordances file](https://minus34.com/opendata/geoscape-202608/boundary_concordance.csv)
...
3. You have a license of [Geoscape Buildings](https://geoscape.com.auhttps://minus34.com/opendata/geoscape-202608/boundary_concordance.csv/buildings/) or [Geoscape Land Parcels](https://geoscape.com.auhttps://minus34.com/opendata/geoscape-202608/boundary_concordance.csv/land-parcels/) and want to use the _planning zone_ data in those products to:
...
1. ABS Census 2026 boundaries ([download](https://minus34.com/opendata/census-2026/census_2026_bdys.dmp))
2. ABS Census 2021 boundaries ([download](https://minus34.com/opendata/census-2021/census_2021_bdys_gda94.dmp))
3. GNAF from gnaf-loader ([download](https://minus34.com/opendata/geoscape-202608/gnaf-202608.dmp))
4. Geoscape Administrative Boundaries from gnaf-loader ([download](https://minus34.com/opendata/geoscape-202608/admin-bdys-202608.dmp))
5. ABS Census 2026 data - used to generate error rates only ([download](https://minus34.com/opendata/census-2026/census_2026_data.dmp))
...
Incorporates or developed using G-NAF © [Geoscape Australia](https://geoscape.com.au/legalhttps://minus34.com/opendata/geoscape-202608/boundary_concordance.csv-copyright-and-disclaimer/) licensed by the Commonwealth of Australia under the [Open Geo-coded National Address File (G-NAF) End User Licence Agreement](https:/https://minus34.com/opendata/geoscape-202608/boundary_concordance.csv.gov.auhttps://minus34.com/opendata/geoscape-202608/boundary_concordance.csvset/ds-dga-19432f89-dc3a-4ef3-b943-5326ef1dbecc/distribution/dist-dga-09f74802-08b1-4214-a6ea-3591b2753d30/details?q=).

Incorporates or developed using Administrative Boundaries © [Geoscape Australia](https://geoscape.com.au/legalhttps://minus34.com/opendata/geoscape-202608/boundary_concordance.csv-copyright-and-disclaimer/) licensed by the Commonwealth of Australia under [Creative Commons Attribution 4.0 International licence (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/).
