...
--     group by target_id
    ...
    select 'abs lga'::text as source_type,
           ...
           (lga.address_count::float / source_counts.address_count::float * 100.0)::smallint  as percent_source_addresses
--            target_counts.address_count                                                       as total_target_addresses,
--            (lga.address_count::float / target_counts.address_count::float * 100.0)::smallint as percent_target_addresses
    from lga
             inner join source_counts on source_counts.source_id = lga.source_id
...
select final.*,
       st_intersection(source_lga.geom, target_lga.geom) as geom
from final
         inner join census_2016_bdys.lga_2016_aust as source_lga on final.source_id = source_lga.lga_code16
         inner join target_lga on final.target_id = target_lga.lga_pid
